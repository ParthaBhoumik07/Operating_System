...
class SharedMemory {
    ...
    public synchronized String readFromReg() throws InterruptedException {   //   * 1. Use monitor for this method [public synchronized String readFromReg()].

        if (pointer == -1) {  // * 2. If the pointer is at -1, it means there is no item in the list. So, suspend the readerThread [using wait()] .
            wait();
        }

        String a = registers.get(0) ; // * 3. Take the value from the the list.

        pointer-- ;  // * 4. Decrease the pointer by 1.

        registers.remove(0) ;  //  * 5. Remove the string from the list,

        return a ;  //  * 6. Return the value
    }

    public synchronized void writeToReg(String value) {  // * 1. Use monitor for this method [public synchronized void writeToReg()].

        String ab = value;

        registers.add(ab) ;  // * 2. Add the string to the list.

        pointer++ ;   // * 3. Increase the pointer by 1.

        if (pointer == 0) {  //  * 4. If the pointer is at 0, it indicates first item in the list. So, wakeup readerThread [using notifyAll()].
            notifyAll();
        }
